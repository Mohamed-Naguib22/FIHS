...
namespace FIHS.Controllers
{
    ...
    public class ArticleController : ControllerBase
    {
        ...
        public async Task<IActionResult> SearchAsync([FromQuery] string topic, [FromQuery] int num)
        {
            var result = await _articleService.SearchAsync(topic, num);
            return result.Succeeded ? Ok(result.Articles) : StatusCode(500, result.Message);
        }

        [HttpGet("get-topics")]
        [ProducesResponseType(StatusCodes.Status200OK)]
        public async Task<IActionResult> GetAllTopicsAsync() => Ok(await _articleService.GetAllTopicsAsync());

        [HttpPost("add-topic")]
        [Authorize(Roles = "Admin")]
        [ProducesResponseType(StatusCodes.Status200OK)]
        [ProducesResponseType(StatusCodes.Status400BadRequest)]
        public async Task<IActionResult> AddTopicAsync([FromBody] Topic topic) 
        {
            var result = await _articleService.AddTopicAsync(topic);

            if (!result)
                return BadRequest("Â–« «·„Ê÷Ê⁄ «·⁄‰Ê«‰ »«·›⁄·");
        ...
        {
            var result = await _articleService.DeleteTopicAsync(id);

            if (!result)
                return NotFound("Â–« «·⁄‰Ê«‰ €Ì— „ÊÃÊœ");
